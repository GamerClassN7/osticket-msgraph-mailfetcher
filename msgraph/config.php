...
class MSGraphPluginConfig extends PluginConfig
{
    ...
    function getOptions()
    {
        ...
        return array(
            ...
            'msgraph-proxyAddress' => new TextboxField(array(
                'label' => $__('Proxy Address'),
                'configuration' => array('size' => 60, 'length' => 60)
            )),
            'msgraph-proxyPort' => new TextboxField(array(
                'label' => $__('Proxy Port'),
    ...
    function pre_save(&$config, &$errors)
    {
        ...
        if (
            empty($config['msgraph-tenantId']) || empty($config['msgraph-clientId']) || (empty($config['msgraph-clientSecret']) && empty($this->get('msgraph-clientSecret')))
        ...
        $rest = new MSGraphAPIREST(
            array(
                ...
                'proxy_address' => $config['msgraph-proxyAddress'],
                'proxy_port' => $config['msgraph-proxyPort'],
            )
        );
