...
class MSGraphPlugin extends Plugin {
	...
	function cron($config) {
		...
		if( empty( $inboxFolderId ) ) {
			$inboxFolderId = $this->rest->request( "users/" . rawurlencode( $config->get( 'msgraph-email' ) ) ."/mailFolders", 'GET', [ 
		...
		if( empty( $processedFolderId ) ) {
			$processedFolderId = $this->rest->request( "users/" . rawurlencode( $config->get( 'msgraph-email' ) ) ."/mailFolders", 'GET', [
		...
		$messages = $this->rest->request( "users/" . rawurlencode( $config->get( 'msgraph-email' ) ) . "/mailFolders/" . $inboxFolderId . "/messages", 'GET', [
